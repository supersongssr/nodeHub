...
| `fix_tiktok_outbound_password.py` | 仅 2026-09-11, 一次 | tiktok 解锁出站密码更换: `/usr/local/etc/xray/config.json` outbounds 中 `address=unlocktiktok.freessr.bid` 出站 (settings.servers, 兼容 vnext/users) 的 `password: fbiopenthedoor → aiopenthedoor`, 重启 xray。无该出站的节点静默跳过并落标记。幂等, 自带备份回滚 + `xray -test` 校验 + Telegram 通知。脚本内部校验日期 (仅 2026-09-11 当天可执行), wrapper 成功后写 marker |

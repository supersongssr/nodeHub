...
设计原则:
  1. 默认只读诊断, 不改系统, 仅两处例外:
     ① 被墙交叉验证 (xcheck): 检出封锁或自检 (原端口大陆全通) 时, 会临时在公网监听
        一个随机端口 (20000-60000, 最长数分钟), 并将节点 IP:端口 提交第三方 tcp.ping.pe
        代测 — 属运行时网络状态变更 (可能触发云安全组/IDS 告警);
        NODE_CN_TCPING_XCHECK=0 可关闭 (含自检), 恢复严格只读;
     ② --fix 显式开启自动修复, 仅限两类低风险项 (apt/dpkg 锁解除 + 仓库有效期校验关闭),
        每个修复动作以 WARN 结果呈现

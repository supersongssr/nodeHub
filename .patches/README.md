...
| 脚本 | 触发窗口 | 作用 |
| --- | --- | --- |
| `fix_traffic_reset_day_and_traffic_used.py` | 2026-08-08 前, 一次 | 读 `~/.env` 的 `NODE_TRAFFIC_RESETDAY` 仅用于确定计费周期起点, 用 vnstat 汇总当前周期 tx 流量, 经 `POST /api/node/edit` **仅上报 `traffic_used`(GiB)**; `traffic_reset_day` 不再上报 (仅作为周期锚点, 不写回面板) |

...
| `proxyDiagnose.sh` | 代理故障一键诊断（xray / nginx / 安装环境 / 网络 / 证书 / 出站连通性 / 本周期流量；含 NODE_PORT 大陆 tcping 被墙检测 —— 借 tcp.ping.pe 三网+云厂探测点与海外对照逐网判定（移动/电信/联通/云厂/海外 各自通/部分/断），检出封锁时随机开临时端口交叉验证【端口级 vs IP 级】被墙，`NODE_CN_TCPING=0` 可整体跳过、`NODE_CN_TCPING_XCHECK=0` 只关交叉验证；亦可在任意第三方服务器上 `NODE_TARGET_IP=<目标IP> NODE_PORT=<端口>` 远测别的节点是否被墙——探测由 ping.pe 代测与运行位置无关，远程目标自动跳过本机监听类检查与交叉验证，完整诊断用 `--host`），支持 `--target`、`--json`、`--no-notify`（抑制自带 Telegram 推送，供 nodeAgent 程序化调度）、`--host` 远程诊断；`--host` ssh 首次连接自动记录主机密钥、指纹变化即拒绝（老版 OpenSSH 可 `DIAG_SSH_STRICT=no` 回退）；`--target traffic` 用 vnstat 统计本周期 tx 出站流量（自上一个 `NODE_TRAFFIC_RESETDAY` 起，配 `NODE_TRAFFIC_LIMIT` 时含限额提醒）；默认只读，不改系统。 |

...
| `proxyDiagnose.sh` | 代理故障一键诊断（xray / nginx / 安装环境 / 网络 / 证书 / 出站连通性 / 本周期流量；含 NODE_PORT 大陆 tcping 被墙检测 —— 借 tcp.ping.pe 大陆探测点与海外对照判定，`NODE_CN_TCPING=0` 可跳过），支持 `--target`、`--json`、`--host` 远程诊断；`--host` ssh 首次连接自动记录主机密钥、指纹变化即拒绝（老版 OpenSSH 可 `DIAG_SSH_STRICT=no` 回退）；`--target traffic` 用 vnstat 统计本周期 tx 出站流量（自上一个 `NODE_TRAFFIC_RESETDAY` 起，配 `NODE_TRAFFIC_LIMIT` 时含限额提醒）；默认只读，不改系统。 |

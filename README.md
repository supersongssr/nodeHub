...
| `nodeAgent.sh` | 节点状态上报（cron 每小时）：采集网卡 rx/tx + uptime + vnstat 近 7 日流量上报面板；每日 03:00 同步 SSL 证书（`SyncSSL`）；每日（默认 05 点后首个周期）检测 `node_port` 是否被墙 —— 借 `proxyDiagnose.sh` 大陆 tcping，仅在【IP 未被墙（交叉验证确认）+ 端口三网全屏蔽】时自动换随机端口重装 `proxyInstall.sh` 快速复活节点，并 Telegram 通知被墙情况与处置结果（`DailyPortBlockCheck`；`NODE_PORT_BLOCK_CHECK=0` 可关闭，`NODE_PORT_CHECK_HOUR` / `NODE_PORT_SWAP_COOLDOWN` 可调，方案见 [`plans/port-block-auto-swap.md`](plans/port-block-auto-swap.md)）；按日期窗口调度一次性补丁（`RunPatches`）。 |
